...
/**
 * File Name:    CurveSampler.java
 * ClassName:    CurveSampler
 *
 * Description: 曲线点采样器
 *
 * @author ZDD
...
public abstract class CurveSampler
{
    ...
    protected static int searchNearestFloor(PointF[] PointFs, int start, float x)
    {
        int low = start;
        int mid = start;
        int high = PointFs.length - 1;
        while (low <= high && PointFs[low].x <= x)
        {
            mid = (low + high) >>> 1;
            float midVal = PointFs[mid].x;
            if (midVal < x)
            {
                low = mid + 1;
            }
